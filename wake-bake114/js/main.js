...
document.addEventListener("keydown", (event) => {
  if (
    event.code === "Escape" &&
    document.body.classList.contains("body__modal-open")
  ) {
    document.body.classList.remove("body__modal-open")
...
function toggleTab(e) {
  ...
  accordionLists.forEach((el) => {
    el.addEventListener("click", (e) => {
      ...
      // Set maxHeight for opening/closing with transition
      if (accordionItem.classList.contains("accordion__list-item--open")) {
        // If the item is open, set the maxHeight to the scrollHeight of the content.
        accordionContent.style.maxHeight = accordionContent.scrollHeight + "px"
      } else {
        // Set maxHeight to null to close (collapse) the content.
